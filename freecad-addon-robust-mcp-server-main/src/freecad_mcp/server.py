...
async def _connect_with_autostart(config: Any) -> None:
    """Attempt connection, auto-starting FreeCAD if necessary."""
    try:
        ...
        logger.warning("Failed to connect to FreeCAD initially: %s", e)
        logger.info("Attempting to auto-start FreeCAD...")
        from freecad_mcp.utils.autostart import AutoStarter
        starter = AutoStarter(host=config.socket_host, xmlrpc_port=config.xmlrpc_port, socket_port=config.socket_port)
        if not starter.start():
            logger.error("Auto-start failed.")
            raise

        global _autostarter
        _autostarter = starter
        connected = False
        import asyncio
        import contextlib
        # Poll much faster (0.2s) instead of 1s to connect immediately upon startup
        for i in range(50):
            with contextlib.suppress(Exception):
                await asyncio.sleep(0.2)
                if i % 5 == 0:
                    logger.info("Retrying connection (attempt %d/50)...", i + 1)
                await _bridge.connect()
                connected = True
                break

        if not connected:
            logger.error("Auto-started FreeCAD but could not connect within timeout.")
            starter.stop()
            raise RuntimeError("Failed to connect to FreeCAD after auto-start.") from e
...
async def lifespan(_server: FastMCP) -> AsyncIterator[None]:
    ...
    # Start connection in background so we don't block MCP protocol initialization
    # This prevents "context deadline exceeded" errors in clients with short timeouts
    connect_task = asyncio.create_task(_connect_with_autostart(config))

    # Wait very briefly just in case it connects immediately without autostart
    ...
    try:
        ...
        # Stop auto-started FreeCAD if we started it
        if '_autostarter' in globals() and _autostarter:
            _autostarter.stop()
